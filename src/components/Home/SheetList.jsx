  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
      {sheets.map(({ title, description, icon, link }) => (
        <div
          key={title}
          className="bg-slate-800 rounded-xl p-8 flex items-start gap-6 shadow-lg border border-slate-700 hover:scale-105 hover:border-cyan-400 transition-all duration-200"
        >
          ...
          <div>
            ...
            <Link
              to={link}
              className="inline-block bg-cyan-400 text-slate-900 font-semibold px-6 py-2 rounded-full shadow hover:bg-cyan-300 transition text-base"
            >
              View Sheet
            </Link>
          </div>
