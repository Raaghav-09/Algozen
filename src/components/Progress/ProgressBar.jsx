...
function ProgressBar({ value, className = "", barClassName = "", children }) {
  return (
    <div
      className={`relative h-3 w-full rounded-full bg-slate-700 overflow-hidden ${className}`}
    >
      <div
        className={`absolute h-full rounded-full bg-cyan-400 transition-all duration-500 ${barClassName}`}
        style={{ width: `${value}%` }}
      />
      {children && (
