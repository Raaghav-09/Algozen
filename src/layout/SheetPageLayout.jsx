...
function SheetPageLayout({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white font-sans py-10 px-4">
      <div className="max-w-3xl mx-auto">
        {/* Header */}
        <header className="flex flex-col md:flex-row items-center justify-between mb-10 gap-6">
          <div className="flex-1 text-center md:text-left">
            <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight mb-2">
              <span className="text-cyan-400 drop-shadow">{title}</span>
            </h1>
            {tagline && (
              <p className="text-lg text-white italic font-medium">{tagline}</p>
            )}
            <div className="mt-4 text-lg text-slate-300 font-semibold">
              Total Progress:{" "}
              <span className="text-cyan-400">{totalSolved}</span>
              <span className="text-slate-400"> / {totalProblems}</span>
            </div>
          </div>
        ...
        <div className="mb-6 flex justify-end">
          <button
            ...
            className={`
              ...
              px-5 py-2.5 rounded-full bg-gradient-to-r from-cyan-400 via-blue-500 to-indigo-500
              text-slate-900 font-bold shadow-lg
          ...
          >
            ...
            <span
              className={`text-xl transition-transform duration-200 ${
                showFavorites
                  ? "rotate-12 scale-110 text-cyan-600"
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center mb-3">
                  <div className="flex-1 mr-4" aria-hidden="true">
                    <div className="relative h-3 w-full rounded-full bg-slate-700 overflow-hidden">
                      <div
                        className="absolute h-full rounded-full bg-cyan-400 transition-all duration-500"
                        style={{ width: `${percent}%` }}
                      />
                    </div>
                  </div>
                ...
                <div
                  ...
                  className={`px-2 pb-2 text-slate-200 text-base transition-all duration-300 ease-in-out ${
                    isOpen ? "block" : "hidden"
                  }`}
                >
                  {links.length > 0 ? (
